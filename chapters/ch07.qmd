...
In the last module we saw how single variables can be described or summarized using descriptive statistics (see @sec-06). In this chapter we will extend that to summarizing values by groups. Indeed, comparing the mean values in different groups is one of the most important types of tests we do in biology (see @sec-11).
...
### Summarizing by groups (AKA: pivot tables) {#sec-07-agg}
...
## Tabulation (frequency tables) {#sec-07-tab}
...
### Contingency tables with `ftable()` {#sec-07-ftable}
...
Notice that the data frame form of `handed.table` resembles the "long" data format we discussed in @sec-05-df-oth-shap.
