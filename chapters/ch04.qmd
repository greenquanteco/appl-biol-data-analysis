...
## Working with your file system {#sec-ie-files}

### Your file system {#sec-ie-files-your}
...
### Folder addresses {#sec-ie-files-address}
...
### The working directory {#sec-ie-files-wd}
...
![Example of hierarchical structure of folders relative to the file system's root. Depending on context, the root can be the system root or the root for a project.](../images/04_01.jpg)
...
::: {layout-ncol="2"}
![To set your working directory, first click on the ... button on the right side of the Files tab.](../images/04_02.jpg)

![Then, select your folder and click More...Set as working directory.](../images/04_03.jpg)
:::
...
### Setting up data files {#sec-ie-format-set}
...
#### Variable names {#sec-ie-format-set-vars}
...
![Examples of bad data management that some simple QA/QC might fix.](../images/04_04.jpg)

## Data import {#sec-ie-import}

### Importing from text files {#sec-ie-import-text}
...
#### Arguments to `read.table()` and `read.csv()` {#sec-ie-import-text-args}
...
The `sep` argument is usually necessary when using the “clipboard trick” (see @sec-ie-import-special)
...
### Saving and importing R objects {#sec-ie-import-object}
...
### Importing from saved workspaces {#sec-ie-import-rdata}

Everything you do in R takes place within the **workspace**. This can be thought of as a sandbox where all of your data and functions reside. If you want to save this entire environment and continue working later you can do this by saving the workspace with `save.image()` (see @sec-ie-export-rdata).
...
1. After simulations are complete, import the results of each simulation from its individual workspace into a new environment.
1. Copy the objects from the environment containing the loaded workspace to the main workspace.
1. Delete the environment containing the loaded workspace.
1. Repeat until all results are imported.
...
### Importing special cases {#sec-ie-import-special}

#### The clipboard trick {#sec-ie-import-special-clip}
...
1. In Excel, highlight your data and copy it to the clipboard using Ctrl-C (or Cmd-C on a Mac).
1. Click in the R console to move your focus there. In R, hit the ENTER key.
1. The command you typed--but did not execute--in step 1 will read the table from the clipboard.
...
#### Entering data using `scan()` {#sec-ie-import-special-scan}
...
#### Entering data directl using `c()` {#sec-ie-import-special-c}
...
## Data export {#sec-ie-export}

### Exporting text files {#sec-ie-export-text}
...
### Saving R objects {#sec-ie-export-robj}
...
### Saving workspaces {#sec-ie-export-rdata}
...
### Exporting graphics {#sec-ie-export-graphics}
...
1. Start a new graphics device using `jpeg()` or `.png` or whichever function is appropriate. This function will include the target path as an argument.
1. Run the code to make your plot.
1. Use `dev.off()` to tell R that your plot is done, and to go ahead and render and save it.
