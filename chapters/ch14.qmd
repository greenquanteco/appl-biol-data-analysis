...
![Comparison of the structure of data frames and matrices in R.](../images/review-02.jpg){#fig-review-dataframe-matrix fig-alt="Side-by-side comparison of a data frame and a matrix. In the data frame, columns are vectors accessed with the dollar-sign operator, while rows are data frames; values within each column must share a type, but different columns may contain different types. In the matrix, columns and rows are vectors accessed with brackets, and all values in the matrix must share the same type. A red outline highlights one column in each structure and a blue outline highlights one row."}
...
is a linear combination of $x$ and $y$, where $a$ and $b$ are constants. The $x$ and the $y$ could be single values, or, as commonly used in statistics, **vectors** of values. A vector in this sense is an ordered set of values. One example of a linear combination that you're already familiar with is a [weighted mean](sec-desc-cen-mean-wm). The weighted mean of a set of numbers $\left[x_1,x_2,\ldots,x_n\right]$ is given by the sum
...
the *t*-test is a linear model, but it doesn’t really describe a line on
a plot of *y* vs. *x* the way that linear regression does. However, if
you encode the factor (grouping variable) it should be clear what the difference in group means really means:

$$
E\left(y_i\right)=\left(1\times\beta_0\right)+\left(x_i\times\beta_1\right)
$$

For group 1, $x_1=0$, so the expected value is $\beta_0$. For group 2,
$x_i=1$, so the $\beta_1$ term comes into play as the difference in
...
![Illustration of how a *t*-test qualifies as a linear model.](../images/review-ttest.jpg){}
...
![Illustration of how a one-way ANOVA with 3 levels qualifies as a linear model.](../images/review-3anova.jpg){}
...
The caret symbol $\hat{}$ indicates that a value is an “estimate”, even
when it is solved for from data (e.g., $\hat{\beta_1}$ is pronounced
“beta-one-hat” and is the estimated slope). Similarly, the bar above a
letter indicates the sample mean (e.g., $\bar{x}$" is pronounced “x bar”
and is the mean of all observed *x*).

#### Assumptions of linear models {#sec-review-lin-assum}
...
The linear model assumes that model residuals, the random errors representing the differences between observed and predicted values, follow a normal distribution. Models for data that do not have normal residuals exist, but they are not linear models. We’ll explore those in @sec-glm1.
...
In this version, the symbol $\sim$ means “distributed as” and denotes a
**stochastic** relationship—a relationship that contains some element of
randomness. This is in contrast to a **deterministic** relationship,
which does not (equality, aka: identity, = is the most famous kind of
deterministic relationship).

##### Assumption 2: Homogeneity of variance

Linear models assume **homoscedasticity**, or constant variance. This
means that the residual variance does not depend on either *x* or
*y*.If the variance in the response variable depends on some predictor variable, then this should be incorporated into the model (resulting in something other thana linear model). If the variance appears to depend on the response variable, then there is probably an issue with the assumed response distribution.
...
![Example of heteroscedasticity, or unequal variance, across the domain of $x$. The left panel shows homoscedasticity, where residual varincee is approximately the same for all $x$. The right panel shows heteroscedasticity, where residual variance increases with $x$](../images/review-homog.jpg){}

The figure below shows two datasets with a categorical predictor,
suitable for a *t*-test. The left panel shows a relationship with equal
variances in each group, while the right panel shows a situation with
unequal variances.

![Example of heteroscedasticity, or unequal variance, between two levels *a* and *b* of some factor. The left panel shows homoscedasticity, where residual varincee is approximately the same in group *a* and group *b*. The right panel shows heteroscedasticity, where residual variance is different in each group.](../images/review-homog2.jpg){}
...
##### Assumption 4: Independently and identically distributed errors
(*i*.*i*.*d*.)
...
##### Final note about linear model assumptions
...
More generally, ANOVA refers to a framework for evaluating how variation is attributed to different components of a statistical model. This is called "partitioning the variance" or "partitioning sums of squares". For example, an ANOVA table can be used to test a linear regression even when the predictor is continuous and there are no groups to compare. The function of ANOVA in that case is to determine how much variation is explained by the different predictors--be they continuous or factors--in the model and how much variation is left over (residual). We will explore this sense of ANOVA in @sec-groups-more-anova-1way.

This broader usage is reflected in R's `anova()` function, which can be applied to many kinds of models. For ordinary linear models, ANOVA tests are based on partitioning sums of squares. For generalized linear models, analogous tests are commonly based on changes in **deviance** rather than sums of squares. Deviance is a generalization of variance (which is calculated from sums of squares), so sums of squares are a special case of deviance (@sec-glm1-dev). This reflects the fact that linear models based on sums of squares are a special case of generalized linear models (@sec-glm1).