...
The **exponential distribution** describes the distribution of waiting times until a single event occurs, given a constant rate per unit time of that event occurring. This makes it the continuous analog of the [geometric distribution](@sec-09-geom). The exponential distribution is also a special case of the [gamma distribution](@sec-09-gamma) where shape parameter $k=1$.
...
In [generalized linear models (GLM)](@sec-16) the response variable is modeled on a scale defined by a **link function**. That link function can look like a transform, and is often confused with a transform, but there is an important difference. 
...
The R command for the probit function is `qnorm()`. As you might suspect from earlier in this chapter (@sec-09-norm-r), the inverse function is `pnorm()`.
...
#### Arcsine square root transformation {#sec-09-trans-asin}
