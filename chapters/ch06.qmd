...
#### Correlations between 2 variables {#sec-desc-co-lin-two}
...
#### Correlations between many pairs of variables  {#sec-desc-co-lin-many}
...
#### Scatterplot matrices {#sec-desc-co-lin-pairs}
...
#### Nonlinear correlations between many pairs of variables  {#sec-desc-co-non-pairs}

Every function that works with linear correlation will also work with nonlinear correlation. For example, we can get a matrix of Spearman correlations by using the `method` argument to `cor()`.

```{r}
# Nonlinear correlation matrix
cor(iris[,1:4], method="spearman")
```

And we can modify the accessory functions of `pairs()` to use the Spearman correlation.

```{r}
#| fig-cap: "Scatterplot matrix showing possible nonlinear correlations between the numeric variables in the `iris` data frame."
#| fig-alt:  "Scatterplot matrix showing possible nonlinear correlations between the numeric variables in the `iris` data frame."
#| label: fig-scatterplotmatrix2
...
{
    par(usr = c(0, 1, 0, 1))
    r <- abs(cor(x, y, method="spearman")) # <-- changed here
    txt <- format(c(r, 0.123456789), digits = digits)[1]
    txt <- paste0(prefix, txt)
    if(missing(cex.cor)) cex.cor <- 0.8/strwidth(txt)
    text(0.5, 0.5, txt, cex = cex.cor * r)
