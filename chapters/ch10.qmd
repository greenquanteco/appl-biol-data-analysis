...
Another way to deal with outliers is to [transform the data](#mod-05-trans). Variance-stabilizing transformations, such as the log-transform, often have the side-effect of reducing outliers in the data.
...
### Graphical methods to identify autocorrelation
...
knitr::include_graphics("07_01.jpg", auto_pdf=TRUE)
```

```{r, label='chunkx520', eval=FALSE, echo=FALSE, fig.width=7, fig.height=5, fig.align="center", message=FALSE, warning=FALSE}
# inspired by http://santiago.begueria.es/2010/10/generating-spatially-correlated-random-fields-with-r/
# and https://www.r-bloggers.com/2019/07/exploring-spatial-autocorrelation-in-r/
...
grid.n <- 100
xy <- expand.grid(x=1:grid.n, y=1:grid.n)
...
# simulated predictor variables based on the gstat object
pred <- predict(g.dummy, newdata=xy, nsim=2)

# autocorrelated response variable
err <- rnorm(nrow(pred), sd=1)
pred$z <- 0.7*pred$sim1 + 1.4*pred$sim2 + err
...
# variogram
vario <- variogram(z~1, data=pred, locations=~x+y)

par(mfrow=c(1,1), mar=c(5.1, 6.1, 1.1, 1.1),
    bty="n", lend=1, las=1, cex.axis=1.3, cex.lab=1.3)
plot(vario$dist, vario$gamma, xlim=c(0, 50), ylim=c(0, 4),
    xlab="Distance", ylab="",
    type="l", lwd=4, yaxt="n", xaxt="n")
axis(side=2, at=0:4, labels=NA, tcl=0)
axis(side=2, at=c(min(vario$gamma), max(vario$gamma)),
    labels=c("Nugget", "Sill"))
axis(side=1, at=0:5*10, labels=NA, tcl=0)
axis(side=1, at=41, labels="Range")
title(ylab="Semivariance", line=5.1)
segments(-1, max(vario$gamma), 41, max(vario$gamma), lty=2)
segments(41, 0.98*max(vario$gamma), 41, -1, lty=2)
...
```{r, fig.width=7, fig.height=5, fig.align="center", collapse=TRUE}
library(vegan)
...
### Option 1: Ignore missing data

The easiest solution is simply to ignore missing data. Consider [this dataset](https://greenquanteco.github.io/missing_data_example.csv), which contains morphological data on 63 species of bats [@hutcheon2002]. This dataset is modified from [an example in the unit on nonlinear models](#mod-11-cart1). Notice that in this version the brain masses (`brw`) are missing for 3 species of bats.
...
### Option 2: Interpolate or impute missing data
